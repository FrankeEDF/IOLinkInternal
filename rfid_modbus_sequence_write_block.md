# MultiIO RFID MIFARE Block Write Operation - Sequence Diagram

## Overview
This document describes the communication sequence for writing a MIFARE block via Modbus in the MultiIO RFID system.
...
!theme plain
title MultiIO RFID MIFARE Block Write Operation - Communication Sequence
...
== MIFARE Block Write Operation ==

PC -> FW: **Modbus Write Multiple Registers**\nFunc: 0x10\nAddr: 1018\nCount: 8 registers\nData: 16 bytes block data

FW -> FW: Trigger RFID block write operation
...
group RFID Communication
    ...
    FW -> RFID: **MIFARE Write Block**\nCmd: 0x18\nBlock Nr: [block_num]\n0x50 00 11 18 [block] [16 bytes data] [crc]
    
    alt Success
        RFID --> FW: **Write Success Response**\n0x50 00 00 73 (ACK)
        FW -> FW: Write operation successful
    else Authentication Failed
        RFID --> FW: **Error Response**\n0xF0 00 02 16 05 [crc]
        FW -> FW: Set lastError = 0x1605
    else Write Failed  
        RFID --> FW: **Error Response**\n0xF0 00 02 18 05 [crc]
        FW -> FW: Set lastError = 0x1805
    end
...
### MIFARE Write Operation
- Triggered by writing to registers 1018-1025
- Performs complete RFID communication sequence:
  - Stops continuous mode (redundant if FB2 already set)
  - Login to MIFARE tag
  - Authenticate sector with selected key
  - Write specified block with data
  - Restart continuous mode
- Returns success or Modbus exception on error

### Error Handling
- Check register 1026 for last error code
- Common error codes:
  - 0x0000: Success
  - 0x1605: Authentication failed
  - 0x1805: Write failed
